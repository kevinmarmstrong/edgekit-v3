...
/**
 * Mission Profile — the recommended way for apps to localize an Edgekit sidecar
 * to a specific mission without tightly coupling their integration code to raw
 * CreateAgentOptions.
 *
 * Edgekit owns the runtime capabilities and primitives.
 * The consuming app owns the "mission profile" — the localized behavior,
 * prompts, tool manifests, synthesis expectations, and defaults for THIS
 * particular use case (shopping assistant, docs Q&A, admin workflows, etc.).
 *
 * Benefits:
 * - Much safer Edgekit upgrades (profiles declare what they depend on).
 * - Clear separation: core changes rarely break well-written profiles.
 * - Dev agents (and humans) can generate, review, and version profiles as first-class artifacts.
 * - Different missions in the same app can have completely different localization.
 */
